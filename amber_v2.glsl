...
const float TAPER_POWER = 3.0;                                     // How dramatically the tail tapers (higher = more dramatic)
const float MIN_TAIL_WIDTH = 0.001;                                // Minimum width at tail end
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (lineLength > 0.001) {
        ...
        for (int i = 0; i < TAIL_SEGMENTS; i++) {
            ...
            // Calculate width with dramatic tapering
            float width1 = currentCursor.z * 0.5 * pow(1.0 - t1, TAPER_POWER);
            float width2 = mix(currentCursor.z * 0.5 * pow(1.0 - t2, TAPER_POWER), MIN_TAIL_WIDTH, t2);
